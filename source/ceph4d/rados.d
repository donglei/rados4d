...
const LIBRADOS_VER_MAJOR = 0;
const LIBRADOS_VER_MINOR = 69;

const LIBRADOS_VER_EXTRA = 1;

const LIBRADOS_SUPPORTS_WATCH = 1;
/* RADOS lock flags
* They are also defined in cls_lock_types.h. Keep them in sync!
*/

const LIBRADOS_LOCK_FLAG_RENEW = 0x1;
/*
* Constants for rados_write_op_create().
*/
const LIBRADOS_CREATE_EXCLUSIVE = 1;

const LIBRADOS_CREATE_IDEMPOTENT = 0;
/*
...
int  rados_create2(rados_t *pcluster, char *clustername, char *name, uint64_t flags);
...
int  rados_ping_monitor(rados_t cluster, char *mon_id, char **outstr,size_t *outstrlen);
...
int  rados_conf_parse_argv_remainder(rados_t cluster, int argc, char **argv, char **remargv);
/**
...
int  rados_conf_parse_env(rados_t cluster, char *var);
...
int  rados_cluster_fsid(rados_t cluster, char *buf,...);
...
int  rados_pool_list(rados_t cluster, char *buf,...);
...
*/
alias void delegate(rados_completion_t cb, void *arg) *rados_callback_t;
